...
data_year <- 9999 ## set at the beginning
...
if (data_year == 2023) {
  ...
  head(mydat)
  str(mydat)
  ## TODO ugly that first column = the rownames - how to suppress that? not visible in editor
  mydat <- mydat[, -1]
  
  
   myclim <- read.table(here("data",
...
if (data_year == 2024) {
  ...
  mydat <- read.table(here("data", "2024_data_mosquitoes_20260109.csv"), 
                    sep = ",", quote = "\"'", dec = ".", header = TRUE)
...
if (data_year == 9999) {
  ...
  myclim <- read.table(here("data",  
                    ...
                    sep = ";", quote = "\"'", dec = ",", header = TRUE)
  myclim <- myclim[, c(1:18)]
  
  write.table(x = myclim, 
              file = here("data","9999_clim.csv"),
              sep = "\t",
              row.names = FALSE,
              col.names = TRUE)
...
## TODO - retreieve temp+hum values now

```



### TODO: put loading of all files into the helper functions 


### TODO - merge with above - this is chaos now and eval = F
```{r, eval = FALSE}
##### -----------------       2023      ------------------------------  #####
...
if (data_year == 2023) {
  ...
  head(mydat)
  str(mydat)
  mydat <- mydat[, -1] ## first column contained 
  
  
      myenv <- read.table(here("data",  
                         "BE23_BUA_correlation_data20260218.csv"), 
                    sep = ";", quote = "\"'", dec = ",", header = TRUE)
      myenv <- myenv[, c(1:18)]
  ...
  if (data_year == 2024) {
    ...
    mydat <- read.table(here("data", "2024_data_mosquitoes_20260109.csv"), 
      ...
      mydat <- read.table(here("data",  
                         "9999_data_mosquitoes_20260219.csv"), 
                    sep = ";", quote = "\"'", dec = ",", header = TRUE)
      mydat <- mydat[, -c(37:39)] ##TODO delete the last three cols
...
## TODO clean code like this

 mymosq <- mymosq[, -1] ## first column contained 
  names(mymosq)[names(mymosq) == 'Sampling_site'] <- 'site'
  

  
  write.table(x = mymosq, 
              file = here("data","2023_data_mosquitoes_20240313.csv"),
              sep = "\t",
              row.names = FALSE,
              col.names = TRUE)
...
if (data_year == 2024) {
  ...
  mydat_no19June_comb2 <- mydat_no19June_comb[,-mycol2] 
  names(mydat_no19June_comb2)
  mydat_no19June_comb_final <- mydat_no19June_comb2[,c(1:23,26:27,24,28)] ##sort unidentified at end
  names(mydat_no19June_comb_final)
...
FLAG_dataset <- 0 # 0 for rarefaction AB mosquitos , 1 for combined datasets

## uncombined, original dataset - not combined - with 17th of June and no/with unidentified
if (FLAG_dataset == 0) {
  mydat_to_run <- mydat_no19June
  colstart <- 11
  colend   <- 30 ## take 31 when unidentified are included in mosqu div analysis, else 31

  colstart_wnv <- 32 
  colend_wnv   <- 32 ## no unidentified WNV
  suffix_for_plot <- '_uncomb_unident_no_'
}
...
if (FLAG_dataset == 1) {
  mydat_to_run <- mydat_no19June_comb_final
  colstart <- 11
  colend   <- 25 ## take 26 when unidentified are included in mosqu div analysis
...
sum_c_wnv <- as.numeric(sum(C[, colstart_wnv], na.rm=T))
...
if (data_year == 2024) {
  wnv_inf_spec <- names(AB_wnv_beta) ## TODO grep WNV with AB
  ab_inf_spec <- c("AB_Culex_pipiens_spp")
 ## TODO again coded with hard wired numbers
  ab_rb <- RB[, c(ab_inf_spec[1])]
  ab_ra <- RA[, c(ab_inf_spec[1])]
  ab_s <- S[, c(ab_inf_spec[1])]
  ab_c <- C[, c(ab_inf_spec[1])]
  ab_n <- N[, c(ab_inf_spec[1])]
...
AB_mosq_infected <- rbind(totsum_rb,totsum_ra,totsum_s,totsum_c,totsum_n) 
rownames(AB_mosq_infected) <- c("RB","RA", "S","C","N")
colnames(AB_mosq_infected) <- "AB_Culex_pipiens_spp"
...
if (data_year == 2024) {
master_mosq <- AB_mosq_infected
master_wnv  <- cbind(AB_wnv_beta,AB_wnv_beta_rel)
colnames(master_wnv) <- c("WNV_Culex_pipiens_spp","MIR")
}
...
##TODO aggregate temperature and humidity
