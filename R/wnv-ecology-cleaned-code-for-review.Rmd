...
ifelse(!dir.exists(file.path(dirfilepath)), 
       dir.create(file.path(dirfilepath)), FALSE)


theme_set(d6::theme_d6(base_size = 18))

site.cols  <- c("#CDCD00", "#6CA6CD","#00EE76","#FF7F50","#006400")
site.names <- c("S",       "RB",      "C",     "RA",     "N")
...
sum_rotha_wnv <- as.numeric(sum(rotha[, colstart_wnv], na.rm=T))
sum_linde_wnv <- as.numeric(sum(linde[, colstart_wnv], na.rm=T))
sum_malzf_wnv <- as.numeric(sum(malzf[, colstart_wnv], na.rm=T))
sum_fried_wnv <- as.numeric(sum(fried[, colstart_wnv], na.rm=T))
sum_suedg_wnv <- as.numeric(sum(suedg[, colstart_wnv], na.rm=T))
}
