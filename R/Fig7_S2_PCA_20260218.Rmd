...
if (length(a) > 0){
 ...
 # col_pos1 <- which(colnames(data_normalized) == "Muscicapa_striata") ## TODO not nice!
 # col_pos2 <- which(colnames(data_normalized) == "Passer_montanus") ## TODO not nice!
 ...
 # data_normalized_b <- data_normalized[,-c(col_pos, col_pos1, col_pos2, col_pos3)]
  data_normalized_b <- data_normalized[,-c(col_pos)]
...
df_sorted$site       ## "C"  "N"  "RA" "RB" "S" 
  ...
  p <- fviz_pca_biplot(data.pca, 
               ...
               # addEllipses = TRUE, label = "var", mean.point = FALSE,
               # ellipse.type = 'confidence', ellipse.level=0.7,
                legend.title = "cos2" ) + #scale_color_manual(values = site.cols)
