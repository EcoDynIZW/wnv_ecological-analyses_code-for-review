...
## ----mir-per-site-------------------------------------------------------------------------------------------------------------
MIR_rb <- round(sum(AB_wnv_list$RB) / sum(AB_mosq_list$RB) * 1000, digits = 2)
MIR_rb
MIR_ra <- round(sum(AB_wnv_list$RA) / sum(AB_mosq_list$RA) * 1000, digits = 2)
MIR_ra
MIR_s <- round(sum(AB_wnv_list$S) / sum(AB_mosq_list$S) * 1000, digits = 2)
MIR_s
MIR_c <- round(sum(AB_wnv_list$C) / sum(AB_mosq_list$C) * 1000, digits = 2)
MIR_c
MIR_n <- round(sum(AB_wnv_list$N) / sum(AB_mosq_list$N) * 1000, digits = 2)
MIR_n
...
if (data_year != 2024) {
  ...
  mir_site_spec <- mapply(
    FUN = function(val1) round(val1 * 1000, digits = 2),
    mir_site_spec,
    SIMPLIFY = FALSE
  )
...
write.csv(
  x = mosq_div0$AsyEst,
  file = paste0(
    dir_output_path, "/Rarefaction_mosq_", data_year,
...
g0 <- ggiNEXT(mosq_div0,
  type = 1, se = TRUE,
  color.var = "Assemblage", facet.var = "None",
  grey = FALSE
) +
...
## better workaround for saving plots
ggsave(paste0(dir_plot_path, "/Fig_S1_rarefaction_mosq_q0_", data_year, suffix_for_plot, today, ".pdf"),
  width = 10, height = 10, plot = g0
)


## problem with dev.off() which is called several times within functions like iNEXT
...
g1 <- ggiNEXT(mosq_div1,
  type = 1, se = TRUE, facet.var = "None", color.var = "Assemblage",
  grey = FALSE
) +
...
ggsave(paste0(dir_plot_path, "/Fig_rarefaction_mosq_q1_", data_year, suffix_for_plot, today, ".pdf"),
  width = 10, height = 10, plot = g1
)
...
ggsave(paste0(dir_plot_path, "/Fig_rarefaction_mosq_q2_", data_year, suffix_for_plot, today, ".pdf"),
  width = 10, height = 10, plot = g2
)
...
if (data_year != 2024) {
  ...
  wnv_r <- ggiNEXT(wnv_div0,
    type = 1, se = TRUE, facet.var = "None",
  ...
  ggsave(paste0(dir_plot_path, "/Fig_rarefaction_wnv_q0_", data_year, suffix_for_plot, today, ".pdf"),
    width = 10, height = 10, plot = wnv_r
  )
...
dimnames(AB_mosq_beta) <- list(
  rownames(AB_mosq_beta),
  gsub(
    pattern = "AB_", replacement = "",
    x = names(ab_mosq)[-1]
  )
)
...
dimnames(AB_wnv_beta) <- list(
  rownames(AB_wnv_beta),
  gsub(
    pattern = "WNV_", replacement = "",
...
## ----hc-mosq, warnings = FALSE, fig.width=6, fig.height=10--------------------------------------------------------------------
...
## programmed after I found out that there is a simple arg to set NA to grey: na_col
...
ggsave(
  paste0(
    dir_plot_path, "/Fig_5A_heat_mosqAB_log_",
    data_year, suffix_for_plot, today, ".pdf"
  ),
  width = 12, height = 10, plot = p_heat
)

# pdf(paste0(dir_plot_path, "/Fig_5A_heat_mosqAB_log_", data_year, suffix_for_plot, today, ".pdf"),
#  width = 12, height = 10
# )
# pheatmap(my_a_log, color = colors2, breaks = bk2, fontsize = 16)
# dev.off()
...
if (data_year != 2024) {
  ...
  ggsave(paste0(dir_plot_path, "/Fig_5B_heat_wnvAB_", data_year, suffix_for_plot, today, ".pdf"),
    width = 10, height = 10, plot = p_heat_wnv
  )
...
if (data_year != 2024) {
  ...
  ggsave(paste0(dir_plot_path, "/Fig_heat_wnv_mir_", data_year, suffix_for_plot, today, ".pdf"),
    width = 12, height = 10, plot = p_heat_wnv_rel
  )
